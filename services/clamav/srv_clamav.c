...
void generate_error_page(av_req_data_t *data,request_t *req){
     ...
     new_size=strlen(clamav_error_message)+strlen(clamav_tail_message)+strlen(data->virus_name)+10;
     ...
     ci_membuf_write(error_page,(char *)clamav_error_message,strlen(clamav_error_message),0);
