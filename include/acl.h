...
/**
 ...
 \ingroup API
 * Access control lists related API. Structures, functions and macros used
 * to define custom acl types, and use access control lists in services
 * and modules.
 */
...
typedef struct ci_acl_type {
    ...
    */
     ...
     *      (if required)
     *
     * This method releases the acl test data,which  allocated using the
...
/**
   ...
   \ingroup ACL
   *
   * Imagine the following access control list defined in the c-icap config
   * file:\n
...
/**
 \brief An access entry object holds an access control list, and can be
 *      connected to linked lists of access entries.
 \ingroup ACL
 *
...
/**
 ...
 \ingroup ACL
 *
 \param access_entry Pointer to the access entries list (a linked list of
 *                   ci_access_entry_t objects)
 \param req pointer to the request object (ci_request_t object)
 \return CI_ACCESS_ALLOW if request matches the access list, CI_ACCESS_DENY
 *       otherwise
 */
 ...
 *
 ...
 \return Pointer to the ci_acl_type_t structure which implements the acl type,
 *       or NULL
 */
